# Лекція 06 Логічне та фізичне проєктування

## Вступ

У попередніх двох лекціях ми пройшли шлях від концептуальної моделі предметної області до нормалізованого набору відношень: спершу побудували ER-модель бібліотечної системи, потім навчились перевіряти й виправляти її на предмет аномалій за допомогою теорії нормалізації. Але нормалізована схема на папері — це ще не працююча база даних. Процес проєктування бази даних складається з кількох етапів, кожен з яких має свою специфіку та призначення, і сьогодні ми розглянемо два останні: логічне та фізичне проєктування, які перетворюють концептуальну модель на конкретну, оптимізовану під навантаження реалізацію в СУБД.

Логічне проєктування передбачає перетворення концептуальної моделі в модель даних, підтримувану конкретною СУБД, але ще не прив'язану до специфічних параметрів продуктивності та зберігання. Фізичне проєктування, у свою чергу, визначає конкретні способи зберігання даних на фізичних носіях з метою оптимізації продуктивності системи. Різниця між ними принципова: логічне проєктування відповідає на питання «які таблиці, стовпці та ключі потрібні, щоб коректно представити модель», а фізичне — на питання «як зробити так, щоб конкретні запити до цих таблиць виконувались швидко на конкретному обсязі даних».

## Логічне проєктування

### Відображення концептуальної схеми в реляційну модель

Процес трансформації концептуальної моделі (зазвичай представленої у вигляді ER-діаграми) в реляційну схему називається логічним відображенням. Цей процес включає систематичне перетворення всіх елементів концептуальної моделі в реляційні конструкції — і, на відміну від нормалізації, тут немає простору для творчості: кожному типу елемента ER-моделі відповідає конкретне, добре визначене реляційне рішення.
...
Зв'язки типу «багато-до-багатьох» вимагають створення окремої асоціативної таблиці (таблиці зв'язків), яка містить зовнішні ключі обох пов'язаних сутностей як частини складеного первинного ключа. Це саме той підхід, який ми вже застосовували в попередній лекції для зв'язку АВТОР-КНИГА через таблицю `book_authors`.
...
Ієрархії спеціалізації/узагальнення в концептуальних моделях відображають відношення «ISA» (is-a) між сутностями — ми розглядали такий приклад (ОСОБА → СТУДЕНТ, ВИКЛАДАЧ) у лекції про концептуальне моделювання. У реляційній моделі, яка не має вбудованого поняття успадкування, існує кілька стратегій їх реалізації, і вибір між ними — це, по суті, вибір компромісу між простотою запитів і чистотою структури.
...
**Переваги:** простота запитів, що стосуються базового класу; висока продуктивність для запитів поліморфного типу; легкість підтримки зв'язків з базовим класом.

**Недоліки:** значна кількість NULL-значень; складність забезпечення цілісності специфічних атрибутів; неефективне використання простору.
...
**Переваги:** відсутність NULL-значень; ясна структура та проста валідація; ефективне використання простору.

**Недоліки:** дублювання структури базових атрибутів; складність поліморфних запитів; складність підтримки змін у базовому класі.
...
**Переваги:** нормалізована структура без дублювання; легкість додавання нових підкласів; ефективна підтримка зв'язків з базовим класом.

**Недоліки:** необхідність JOIN-операцій для отримання повної інформації; складність забезпечення того, що кожна особа належить рівно одному підкласу.

На практиці варто обирати стратегію 3 за замовчуванням (вона найкраще узгоджується з нормалізацією, яку ми вивчали в попередній лекції), і свідомо переходити до стратегії 1 чи 2 лише тоді, коли є конкретна причина — наприклад, потреба в дуже швидких поліморфних запитах або, навпаки, у повністю ізольованих таблицях без жодного JOIN.
...
**Переваги природних ключів:** мають бізнес-значення та зрозумілі користувачам; не потребують додаткових механізмів генерації; можуть використовуватися в звітах без додаткових JOIN-операцій; забезпечують природні зв'язки між пов'язаними системами.

**Недоліки природних ключів:** можуть змінюватися з часом через бізнес-вимоги; іноді складні або громіздкі для використання; можуть мати проблеми з конфіденційністю; ризик дублювання при інтеграції систем.
...
**Переваги штучних ключів:** ніколи не змінюються, забезпечуючи стабільність зв'язків; завжди унікальні та компактні; не розкривають бізнес-інформацію; ефективні для індексування та JOIN-операцій; підтримують розподілене генерування (UUID).

**Недоліки штучних ключів:** не мають бізнес-значення; потребують додаткової пам'яті; можуть ускладнити налагодження; вимагають додаткових індексів на природні атрибути.

#### Гібридний підхід

Часто оптимальним рішенням є комбінування штучних та природних ключів: штучний ключ обслуговує внутрішні зв'язки між таблицями, а природний зберігається як окреме унікальне поле для бізнес-використання.
...
Нормалізована схема, отримана за правилами попередньої лекції, гарантує відсутність аномалій — але не гарантує швидкості запитів. Денормалізація є процесом навмисного відходу від нормалізованої структури даних з метою покращення продуктивності запитів. Хоча нормалізація зменшує надмірність та покращує цілісність даних, вона може призводити до складних JOIN-операцій, які знижують продуктивність. Важливо розуміти: денормалізація — це свідомий компроміс, який приймається після нормалізації й з повним усвідомленням її ціни, а не заміна нормалізації через незнання.

#### Мотивація для денормалізації

**1. Продуктивність запитів.** Нормалізовані структури часто вимагають множинних JOIN-операцій для отримання даних, що може бути повільним для великих обсягів даних.
...
**2. Спрощення звітності.** Складні аналітичні запити можуть значно спроститися при денормалізованій структурі.

**3. Зменшення навантаження на систему.** Менша кількість JOIN-операцій означає менше навантаження на CPU та I/O підсистему.

#### Техніки денормалізації

**1. Дублювання атрибутів.** Копіювання часто запитуваних атрибутів з пов'язаних таблиць.
...
**2. Передобчислені агрегати.** Зберігання результатів обчислень для уникнення повторних розрахунків.
...
**3. Розгортання ієрархій.** Перетворення ієрархічних структур у плоскі таблиці.
...
**4. Горизонтальне дублювання.** Розбиття великих таблиць на менші за певними критеріями.
...
**1. Проблеми цілісності даних.** Дублювання даних створює ризик неузгодженості.

**2. Ускладнення оновлень.** Зміни повинні поширюватися на всі дубльовані місця.

**3. Збільшення обсягу даних.** Дублювання збільшує потребу в дисковому просторі.

**4. Складність підтримки.** Денормалізовані структури важче розуміти та підтримувати.

Логічна схема — з обраними стратегіями ключів, реалізованими ієрархіями та, за потреби, точково денормалізована — готова. Далі настає етап, де рішення приймаються вже не про структуру даних, а про те, як саме СУБД зберігатиме та читатиме ці дані на диску.

## Фізичне проєктування

Фізичне проєктування визначає, як логічна модель даних буде реалізована на конкретній апаратній та програмній платформі. Цей етап критично важливий для забезпечення необхідної продуктивності системи.
...
**1. Покращення локальності даних.** Пов'язані рядки зберігаються фізично поруч, що зменшує кількість дискових операцій.

**2. Ефективніші діапазонні запити.** Запити за діапазонами значень виконуються швидше.

**3. Покращення стиснення.** Подібні дані стискаються краще.

#### Недоліки кластеризації

**1. Накладні витрати на підтримку.** Вставки можуть бути повільнішими через необхідність підтримання порядку. Важливо також пам'ятати, що `CLUSTER` у PostgreSQL — це одноразова фізична перебудова таблиці, а не властивість, яка підтримується автоматично.

**2. Потреба в перекластеризації.** З часом ефективність кластеризації знижується, і команду доводиться повторювати.
...
**1. Покращення продуктивності запитів.** Запити можуть виконуватися тільки на релевантних секціях.

**2. Простіше обслуговування.** Можна виконувати операції обслуговування на окремих секціях.

**3. Паралельна обробка.** Різні секції можуть оброблятися паралельно.

**4. Ефективне видалення старих даних.** Можна швидко видалити цілу секцію замість окремих рядків.
...
Фізичне проєктування вимагає постійного моніторингу та оптимізації: жодне рішення про індекси чи секціонування не варто вважати остаточним без перевірки на реальному навантаженні.
...
5. **Моніторинг у продукції** — встановіть системи моніторингу для відстеження продуктивності.
...
- **Еволюційний підхід** — проєктування бази даних є ітеративним процесом.
- **Документування рішень** — завжди документуйте причини прийняття проєктних рішень.
...
-- (у PostgreSQL 13 і новіших назви колонок — total_exec_time / mean_exec_time;
--  для версій до 13 використовуйте total_time / mean_time)
...
### Інструменти для логічного проєктування

**1. ERD (Entity-Relationship Diagram) інструменти**

- **MySQL Workbench** — безкоштовний інструмент з можливістю зворотного інжинірингу;
- **PostgreSQL pgAdmin** — вбудовані можливості візуального проєктування;
- **Draw.io / Lucidchart** — вебінструменти для створення діаграм;
- **ERwin Data Modeler** — професійний інструмент для корпоративного моделювання.

Як і в лекції про концептуальне моделювання, тут варто згадати й підхід «схема як код»: міграційні інструменти нижче фактично виконують ту саму роль ERD-редактора, тільки описану текстом і придатну для рев'ю в Git.
...
### Інструменти для фізичного проєктування

**1. Моніторинг продуктивності**

```sql
-- Аналіз повільних запитів у PostgreSQL
-- (колонки total_exec_time / mean_exec_time — актуальні назви для PostgreSQL 13+;
--  у версіях до 13 використовувались total_time / mean_time)
SELECT
    query,
    calls,
    total_exec_time,
...
-- (використовує total_exec_time / mean_exec_time — актуальні назви для PostgreSQL 13+)
...
BEGIN
    ...
    SELECT
        ...
        ROUND(pss.total_exec_time / 1000.0 / 60.0, 2) as total_time_minutes,
        ROUND(pss.mean_exec_time, 2) as avg_time_ms,
        pss.calls,
        CASE
            WHEN pss.mean_exec_time > 1000 THEN 'Consider adding indexes or query optimization'
            WHEN pss.calls > 10000 AND pss.mean_exec_time > 100 THEN 'High frequency query needs optimization'
            WHEN pss.total_exec_time > 300000 THEN 'Major contributor to system load'
            ELSE 'Monitor performance'
        END as optimization_suggestion
    FROM pg_stat_statements pss
    WHERE pss.total_exec_time > 60000 -- більше 1 хвилини загального часу
...
**1. Ітеративне проєктування**

Процес проєктування повинен бути ітеративним, з постійним уточненням та покращенням схеми на основі нових вимог та даних про продуктивність.

```mermaid
graph TD
    A[Аналіз вимог] --> B[Концептуальне моделювання]
    B --> C[Логічне проєктування]
    C --> D[Фізичне проєктування]
...
Створення прототипів з реальними або близькими до реальних обсягами даних для тестування проєктних рішень.
...
Логічне та фізичне проєктування баз даних є критично важливими етапами розробки інформаційних систем, які значною мірою визначають їх продуктивність, масштабованість та підтримуваність.

### Ключові принципи логічного проєктування
...
4. **Урахування майбутніх потреб** — проєктуйте з урахуванням можливого розширення та еволюції системи.

### Ключові принципи фізичного проєктування

1. **Урахування характеристик навантаження** — проєктування повинно базуватися на реальних патернах використання даних.
2. **Ітеративна оптимізація** — фізичне проєктування є процесом постійного вдосконалення на основі моніторингу.
...
- Документуйте всі проєктні рішення та їх обґрунтування.
- Використовуйте системи контролю версій для схем баз даних.
- Регулярно переглядайте та оновлюйте фізичне проєктування.

Правильне проєктування на логічному та фізичному рівнях створює міцну основу для побудови ефективних, масштабованих та підтримуваних систем управління даними, які можуть адаптуватися до змінних потреб бізнесу та зростання обсягів інформації. Разом три лекції — концептуальне моделювання, нормалізація, логічне й фізичне проєктування — утворюють повний цикл проєктування бази даних: від розуміння предметної області до готової, оптимізованої реалізації, з якою ви працюватимете в подальших лабораторних роботах курсу.
