# Лекція 05 Теорія нормалізації реляційних схем

## Вступ

У попередній лекції ми побудували концептуальну модель предметної області та намітили її перетворення на реляційні таблиці. Але саме по собі механічне перетворення сутностей у таблиці ще не гарантує якісної схеми: одну й ту саму концептуальну модель можна реалізувати десятками різних наборів таблиць, і більшість із них виявляться поганими — не тому, що порушують якийсь синтаксис SQL, а тому, що провокують суперечливі та надлишкові дані. Теорія нормалізації дає формальний, а не інтуїтивний спосіб відрізнити хорошу реляційну схему від поганої.

Нормалізація є одним з найважливіших процесів при проєктуванні реляційних баз даних. Цей процес дозволяє структурувати дані таким чином, щоб мінімізувати надмірність, уникнути аномалій при оновленні та забезпечити цілісність даних. Теорія нормалізації, розроблена Едгаром Коддом та розвинена його послідовниками, ґрунтується на математично строгих принципах і дає практичні інструменти для створення якісних схем баз даних.

Основна ідея нормалізації полягає в декомпозиції (розкладанні) таблиць на менші таблиці, які зв'язані між собою через зовнішні ключі. Це дозволяє зберігати кожен факт тільки в одному місці, що значно спрощує підтримку консистентності даних. У цій лекції ми пройдемо весь шлях цієї теорії: від практичних симптомів поганого проєктування (аномалій) через формальний апарат функціональних залежностей і правил Армстронга до ієрархії нормальних форм і алгоритмів декомпозиції, якими ці форми досягаються на практиці.

## Аномалії в ненормалізованих відношеннях

Перш ніж занурюватись у формальний апарат нормалізації, корисно побачити, які саме проблеми вона розв'язує. Ці проблеми називаються аномаліями, і саме вони є основною мотивацією для всього подальшого викладу — без розуміння того, що́ саме йде не так у ненормалізованій таблиці, правила нормальних форм виглядатимуть як довільна абстрактна вправа.
...
Ця єдина таблиця виглядає простою і навіть зручною для перегляду одним запитом — саме тому подібні «широкі» таблиці так часто з'являються в перших чорнових версіях схем. Розглянемо, чому вона насправді проблематична.
...
**1. Неможливість додати новий курс без студента.** Якщо університет вводить новий курс «CS103 — Алгоритми», який викладає «Сидоренко В.М.», але на нього ще ніхто не записався, то неможливо додати цю інформацію в таблицю, оскільки первинний ключ включає `student_id`.

**2. Дублювання інформації про студента.** При реєстрації студента на новий курс доводиться повторно вводити всю інформацію про студента (ім'я, адресу), що створює надмірність та ризик помилок.
...
**1. Зміна назви курсу.** Якщо курс CS101 змінює назву з «Програмування» на «Основи програмування», потрібно оновити ВСІ записи, які містять цей курс. При цьому є ризик пропустити деякі записи або створити неконсистентний стан, де один курс має різні назви.

**2. Зміна адреси студента.** Якщо студент Іван Петров змінює адресу з «вул. Лесі 5» на «вул. Шевченка 10», потрібно оновити ВСІ записи з цим студентом.
...
**1. Втрата інформації про курс.** Якщо студент Петро Сидоров (єдиний студент на курсі CS102) відраховується з університету, то при видаленні його запису втрачається вся інформація про курс «Бази даних» та викладача «Петренко Б.І.».

**2. Втрата інформації про викладача.** При видаленні останнього студента з курсу зникає інформація про те, що цей курс викладає конкретний викладач.

Приклад втрати інформації. Видаляємо запис:

```
| 1003 | Петро Сидоров | вул. Рівненська 8| CS102 | Бази даних | Петренко Б.І. | 3 |
```

Якщо це був єдиний запис з курсом CS102, то втрачається інформація, що курс CS102 має назву «Бази даних» і що його викладає Петренко Б.І.

### Кількісна оцінка проблем надмірності

Проблема аномалій — не лише в незручності, а й у прямій вартості надмірності. Розглянемо кількісний аспект на наведеному прикладі:
...
Забігаючи наперед — кожна нормальна форма, яку ми розглянемо далі, усуває певний тип аномалій:
...
**Вартість зберігання:** надмірні дані займають додатковий дисковий простір, зростають вимоги до пам'яті для кешування, збільшується час створення резервних копій.

**Вартість обслуговування:** більше часу на оновлення даних, ризик помилок при множинних оновленнях, складність забезпечення консистентності.

**Вартість розробки:** додаткова логіка для підтримки консистентності, складніші процедури валідації даних, більше тестових сценаріїв.

Розуміння цих аномалій є ключовим для усвідомлення важливості процесу нормалізації. Але щоб діагностувати аномалії формально, а не «на око», нам потрібен точний інструмент опису залежностей між атрибутами — функціональні залежності.
...
Саме залежність №3 (group_number → group_curator), як ми побачимо далі, і є джерелом транзитивної залежності, яку усуває третя нормальна форма — тримайте цей приклад у пам'яті.
...
**Нетривіальні функціональні залежності** — це залежності, де Y ⊄ X. Саме такі залежності є корисними для проєктування баз даних.
...
Функціональні залежності, виявлені при аналізі предметної області, — це лише вихідний набір. З них можна логічно вивести й інші залежності, які так само обов'язково виконуються, — і робити це вручну, перебираючи варіанти, непрактично. Вільям Армстронг у 1974 році сформулював набір правил, які дозволяють виводити нові функціональні залежності з уже відомих. Ці правила є повними та коректними, тобто дозволяють отримати всі можливі функціональні залежності і тільки правильні.

#### Основні правила (аксіоми)

**Правило рефлексивності (Reflexivity).** Якщо Y ⊆ X, то X → Y. Це правило говорить, що множина атрибутів завжди функціонально визначає свою підмножину.

**Правило нарощування (Augmentation).** Якщо X → Y, то XZ → YZ для будь-якої множини атрибутів Z. Це означає, що до обох частин функціональної залежності можна додати однакову множину атрибутів.

**Правило транзитивності (Transitivity).** Якщо X → Y та Y → Z, то X → Z. Транзитивність дозволяє «ланцюжити» функціональні залежності.

#### Додаткові правила (можуть бути виведені з основних)

**Правило об'єднання (Union).** Якщо X → Y та X → Z, то X → YZ.

**Правило розкладання (Decomposition).** Якщо X → YZ, то X → Y та X → Z.

**Правило псевдотранзитивності (Pseudotransitivity).** Якщо X → Y та YZ → W, то XZ → W.
...
Зауважте, що перший висновок — student_id → group_curator — це саме той транзитивний ланцюжок student_id → group_number → group_curator, який ми відзначили раніше як джерело проблем із 3НФ. Щоб систематично працювати з такими висновками для великих наборів атрибутів, зручно обчислювати не окремі виведення, а повне замикання.
...
Оскільки замикання {A}⁺ охопило всі атрибути відношення, A є суперключем R — факт, який знадобиться нам далі при перевірці нормальних форм.
...
Мінімальне покриття дозволяє представити множину функціональних залежностей у найбільш компактному вигляді — і саме воно є вихідними даними для алгоритму синтезу 3НФ, який ми розглянемо в кінці лекції.

Маючи формальний апарат функціональних залежностей, ми готові дати точні визначення нормальним формам, попередньо лише окреслені як «усувають той чи інший тип аномалій».
...
Розкладена таким чином таблиця вже атомарна, але вона поки що ілюструє нову проблему: student_name повторюється для кожного предмета студента. Саме цю надлишковість усуває наступна нормальна форма.
...
Повна функціональна залежність означає, що атрибут залежить від всього ключа, а не від його частини. Це вимога свідомо стосується лише відношень із композитним первинним ключем — якщо ключ складається з одного атрибута, часткової залежності від нього просто не може існувати, і 2НФ виконується автоматично.
...
Транзитивна залежність виникає, коли неключовий атрибут A залежить від ключа K через інший неключовий атрибут B: K → B → A. Це саме той сценарій student_id → group_number → group_curator, який ми розглядали в розділі про функціональні залежності.
...
3НФ виявляється недостатньою в одному специфічному випадку — коли у відношенні є кілька ключів-кандидатів, що перетинаються за атрибутами. Саме для цього випадку сформульовано строгіше правило.
...
3НФ і НФБК усувають усі аномалії, які походять від функціональних залежностей. Але існує ще один тип семантичного зв'язку між атрибутами, який функціональні залежності взагалі не описують, — і саме він мотивує наступну пару нормальних форм.
...
Багатозначна залежність виникає, коли один атрибут визначає множину значень іншого атрибута, незалежно від решти атрибутів у відношенні. Важливо: таке відношення вже може перебувати в НФБК (жодна з його функціональних залежностей не порушена — адже прямих функціональних залежностей тут узагалі немає), і водночас лишатися надлишковим.
...
Тут student_id ↠ hobby та student_id ↠ language, оскільки хобі та мови студента не залежать одне від одного — саме тому кожна пара «хобі × мова» змушена дублюватись у таблиці.
...
4НФ покриває випадок, коли відношення розкладається без втрат рівно на дві проекції. Але існують — рідкісні на практиці — відношення, які втрачають безвтратність саме при розкладанні на дві частини, але відновлюються без втрат при розкладанні на три чи більше. Для опису такого випадку вводиться залежність з'єднання.

### Залежності з'єднання

**Визначення.** Відношення R має залежність з'єднання ⋈(R₁, R₂, ..., Rₙ), якщо R = R₁ ⋈ R₂ ⋈ ... ⋈ Rₙ, де R₁, R₂, ..., Rₙ є проєкціями відношення R.

Залежність з'єднання описує ситуацію, коли відношення може бути точно відтворене з'єднанням своїх проєкцій.

### П'ята нормальна форма (5НФ)

**Визначення.** Відношення знаходиться в п'ятій нормальній формі (також називається проєкційно-з'єднувальною нормальною формою), якщо:
...
то таблицю можна розкласти на три проєкції без втрат.

На цьому теоретична ієрархія нормальних форм завершується. Лишається практичне питання: як за наявною таблицею та набором функціональних залежностей систематично, а не навмання, отримати потрібну декомпозицію?
...
2. **Збереження залежностей** — усі функціональні залежності повинні бути збережені в результаті декомпозиції.

Ці дві вимоги не завжди сумісні одночасно для НФБК (іноді доводиться жертвувати збереженням залежностей заради безвтратності), тому на практиці існують окремі алгоритми з різними гарантіями.

### Алгоритм декомпозиції до 3НФ зі збереженням залежностей
...
Денормалізація завжди має бути обґрунтованою та супроводжуватися додатковими механізмами підтримки консистентності. Ми детально розглянемо конкретні техніки денормалізації та компроміси, які вона несе, у наступній лекції — коли вже матимемо нормалізовану логічну схему, яку є що свідомо денормалізувати.

## Висновки

Теорія нормалізації є фундаментальним інструментом проєктування якісних реляційних баз даних. Основні принципи, які слід пам'ятати:
...
4. **Алгоритми декомпозиції** забезпечують систематичний підхід до розкладання відношень зі збереженням важливих властивостей.

Правильне застосування принципів нормалізації дозволяє створювати бази даних, які легко підтримувати, мають мінімальну надмірність та високу цілісність даних. Водночас важливо знайти баланс між теоретичною досконалістю та практичними потребами конкретної системи. У наступній лекції ми застосуємо нормалізовану логічну схему як відправну точку для фізичного проєктування — включно з тим, коли й навіщо свідомо відступати від нормальної форми заради продуктивності.
